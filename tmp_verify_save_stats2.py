import requests, urllib3, random, time, sys
...
for i in range(60):
    try:
        ...
        print(f'[{i}] html_version={hv}', flush=True)
        if hv == '2026.07.30.10':
            break
    except Exception as e:
        print(f'[{i}] wait: {e}', flush=True)
    time.sleep(5)
else:
    print('WARN: 版本尚未更新，仍先測 save-stats API（舊後端也有此端點）', flush=True)
...
resp = requests.post(BASE+'/eeg/save-stats', headers=h, json=payload, verify=False, timeout=90)
print(f'\nsave-stats HTTP {resp.status_code}', flush=True)
print(resp.text[:600], flush=True)
if resp.status_code != 200:
    sys.exit(1)
d = resp.json()
sid = d.get('session_id')
print(f'session_id={sid} firebase={d.get("firebase_session_id")} sync={d.get("firebase_sync_ok")}', flush=True)
...
print(f'在 sessions 列表找到: {found is not None}', flush=True)
if found:
    print(f"  name={found.get('subject_name')} captures={found.get('total_captures')}", flush=True)

ver2 = requests.get(BASE + '/app/version', verify=False, timeout=8).json()
print(f"最終版本: {ver2.get('html_version')}", flush=True)
