...
def _session_to_brainwave_data(db: Session, session_id: int, skip_firebase: bool = False) -> Optional[dict]:
    ...
    # ── bands_7：只在 BrainDNA 180 筆計算成功時才填入 0-100 值 ──────────────
    # 若計算失敗（_bdna_bands is None），bands_7 = {} → 前端 fallback 到 bands_avg 原始值
    # 不做 avg_approx fallback（每 session 跑 compute_all 會讓 overview 端點超時）
    _bands_7 = {}
    if _bdna_bands:
        _bands_7 = {
            "theta":      _bdna_theta,
