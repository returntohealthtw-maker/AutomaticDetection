...
# proportionRange 各頻段閾值（依 11 筆實測資料校正）
# level1 = 族群中位數佔比 → 得分 50 分的基準線（~一半人低於此）
# level2 = 族群高分段（~前 10%）→ 接近 100 分的門檻
# 實測佔比範圍：delta 42~57%, theta 11~17%, alpha/beta/gamma 2~10%
_PROP_RANGE = {
    "r_delta":  (0.44, 0.60),    # 中位數 ~46.8%，高分段 ~57%
    "r_theta":  (0.13, 0.19),    # 中位數 ~13.8%，高分段 ~17%
    "r_lalpha": (0.035, 0.065),  # 中位數 ~4.0%，高分段 ~5.9%
    "r_halpha": (0.030, 0.060),  # 中位數 ~3.2%，高分段 ~5.0%
    "r_lbeta":  (0.028, 0.055),  # 中位數 ~3.1%，高分段 ~4.9%
    "r_hbeta":  (0.050, 0.110),  # 中位數 ~5.7%，高分段 ~9.7%
    "r_lgamma": (0.033, 0.075),  # 中位數 ~3.7%，高分段 ~7.0%
    "r_hgamma": (0.022, 0.055),  # 中位數 ~2.6%，高分段 ~5.2%
}
