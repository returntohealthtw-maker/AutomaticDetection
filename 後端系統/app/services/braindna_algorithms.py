...
# proportionRange 各頻段閾值（依 17 筆實測資料重新校正，2026-07-13；low_alpha 擴大 2026-07-30）
# level1 = p50（中位數）→ 得分 50 分的基準線
# level2 = 觀測最大值 × 1.5（充裕空間，確保一般人不輕易滿分）
# 實測分佈（17 sessions；2026-07-30 更新：加入真實 session #124 low_alpha=8.2% 修正 level2）：
#   Delta  39.8~54.4%  p50=47.2%  max=54.4% → level2=60%（保留寬裕）
#   Theta  11.3~20.0%  p50=14.7%  max=20.0% → level2=26%
#   Low α   2.4~ 8.2%  p50= 4.1%  max= 8.2% → level2= 8.2*1.35≈11%（原8%過低，實測8.2%已超出→得100）
#   High α  1.7~6.4%   p50= 3.4%  max= 6.4% → level2= 8%
#   Low β   1.5~4.3%   p50= 3.0%  max= 4.3% → level2= 6%
#   High β  2.6~8.0%   p50= 4.1%  max= 8.0% → level2=10%
#   Low γ   1.1~5.1%   p50= 2.8%  max= 5.1% → level2= 7%
#   High γ  0.7~4.3%   p50= 2.0%  max= 4.3% → level2= 6%
_PROP_RANGE = {
    "r_delta":  (0.47, 0.60),    # p50=47.2%，level2 保留寬裕
    "r_theta":  (0.14, 0.26),    # p50=14.7%，level2 從 19%→26%（修正滿分問題）
    "r_lalpha": (0.040, 0.110),  # p50= 4.1%，level2 從 8%→11%（實測max=8.2%，原level2過低→正常人得100）
    "r_halpha": (0.033, 0.090),  # p50= 3.4%，level2 從 8%→9%（同步略調）
...
_CHILD_PR_2_5: Dict[str, tuple] = {
    ...
    # 2026-07-27 校準（90s視窗）：level2 = 文獻P95 × 1.35，避免最佳90s視窗假性滿分
    "r_delta":  (0.46, 0.56),
    "r_theta":  (0.17, 0.30),   # 文獻P95=22.2% × 1.35 ≈ 30%
    "r_lalpha": (0.056, 0.100), # 文獻P95=8.2% × 1.35 ≈ 11%
    "r_halpha": (0.056, 0.100),
    "r_lbeta":  (0.060, 0.130), # 文獻P95=12.2% × 1.35 ≈ 16%（2-5歲beta個體差異大）
    "r_hbeta":  (0.060, 0.130),
    "r_lgamma": (0.025, 0.080), # 實測估計（加寬）
    "r_hgamma": (0.015, 0.050), # 實測估計
}
...
_CHILD_PR_6_9: Dict[str, tuple] = {
    ...
    # 2026-07-27 診斷（Session #113，9歲，90s最佳視窗實測）：
    #   Theta實測=20.10%，Low-a=8.71%，High-a=8.36%，Low-b=6.82%，High-b=6.37%
    # level2 = 實測值 × 1.5（留足夠空間，避免假性滿分）
    "r_delta":  (0.44, 0.54),
    "r_theta":  (0.17, 0.28),   # 實測20.10% × 1.4 ≈ 28%
    "r_lalpha": (0.062, 0.120), # 實測8.71% × 1.4 ≈ 12%
    "r_halpha": (0.062, 0.120), # 實測8.36% × 1.4 ≈ 12%
    "r_lbeta":  (0.065, 0.110), # 實測6.82%，留寬裕空間
    "r_hbeta":  (0.065, 0.110), # 實測6.37%
    "r_lgamma": (0.025, 0.080), # 實測5.10% × 1.5 ≈ 8%
    "r_hgamma": (0.015, 0.050), # 實測2.98% × 1.5 ≈ 4.5%
}
...
_CHILD_PR_10_13: Dict[str, tuple] = {
    ...
    # 2026-07-27 校準（90s視窗）：level2 = 文獻P95 × 1.35
    "r_delta":  (0.40, 0.53),
    "r_theta":  (0.18, 0.30),   # 文獻P95=23.5% × 1.35 ≈ 32%（用30%較保守）
    "r_lalpha": (0.071, 0.125), # 文獻P95=10.1% × 1.35 ≈ 13.6%（用12.5%較保守）
    "r_halpha": (0.071, 0.125),
    "r_lbeta":  (0.073, 0.120), # 文獻P95=10.9% × 1.35 ≈ 14.7%（用12%較保守）
    "r_hbeta":  (0.073, 0.120),
    "r_lgamma": (0.025, 0.080), # 實測估計
    "r_hgamma": (0.015, 0.050), # 實測估計
}
